...
class ReadStream extends EventEmitter {
    ...
    read(){//读取文件
        if(typeof this.fd != 'number'){
            return this.once('open',()=>this.read())//第一次读取没有获取到fd，监听一下open，open之后执行read
        }
        ...
        fs.read(this.fd,this.buffer,0,howMuchToRead,this.position,(err,byteRead)=>{
            ...
            let b = this.encoding ? this.buffer.slice(0,byteRead).toString(this.encoding) : this.buffer.slice(0, byteRead)//截取真实读到的数据
            this.emit('data',b)
